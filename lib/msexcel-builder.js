...
Workbook = (function() {
  ...
  Workbook.prototype.save = function(cb) {
    ...
    async.series([
      async.apply(fs.copy, path.join(opt.tmpl_path, 'tmpl'), target),
      async.apply(fs.mkdir, path.join(target, 'xl', '_rels')),
      async.apply(fs.mkdir, path.join(target, 'xl', 'worksheets')),
      async.apply(fs.writeFile, path.join(target, '[Content_Types].xml'), self.ct.toxml()),
      async.apply(fs.writeFile, path.join(target, 'docProps', 'app.xml'), self.da.toxml()),
      async.apply(fs.writeFile, path.join(target, 'xl', 'workbook.xml'), self.wb.toxml()),
      async.apply(fs.writeFile, path.join(target, 'xl', 'sharedStrings.xml'), self.ss.toxml()),
      async.apply(fs.writeFile, path.join(target, 'xl', '_rels', 'workbook.xml.rels'), self.re.toxml()),
      async.apply(fs.writeFile, path.join(target, 'xl', 'styles.xml'), self.st.toxml()),
      function(cb) {
        async.forEach(sheetList, function(sheet, sheetDone) {
          fs.writeFile(path.join.apply(this, sheet.name), sheet.data, sheetDone);
        }, function(err) {
          cb(err);
        });
      }
      ...
      var zip = new EasyZip();
      // path.join doesn't handle dots in the path, which we need here
      zip.zipFolder(target + path.sep + ".", function(){
